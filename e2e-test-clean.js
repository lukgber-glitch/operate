...
(async () => {
  console.log('======================================================================');
  console.log('E2E COMPREHENSIVE USER FLOW TEST');
  console.log('======================================================================');
  
  const screenshotDir = path.join(__dirname, 'test-screenshots', 'e2e-comprehensive');
  const cookiesPath = path.join(__dirname, 'test-cookies.json');
  const resultsPath = path.join(__dirname, 'E2E_COMPREHENSIVE_RESULTS.json');
  ...
  const results = {
    testSuite: 'E2E Comprehensive User Flow',
...
Test: Chat Page');
  try {
    ...
    await page.waitForTimeout(3000);
    await screenshot(page, '10-chat', 'Chat page');
    results.pages.push({ name: 'Chat', status: 'PASS', url: page.url() });
    results.summary.passed++;
    console.log('  PASS - Chat page loaded');
    await page.close();
  } catch (err) {
    results.pages.push({ name: 'Chat', status: 'FAIL', error: err.message });
    results.summary.failed++;
    console.log('  FAIL -', err.message);
  }
  results.summary.total++;
  ...
  // Test Finance Pages
  const financePages = [
    { name: 'Invoices', url: '/finance/invoices', screenshot: '11-invoices' },
    { name: 'Expenses', url: '/finance/expenses', screenshot: '12-expenses' },
    { name: 'Time', url: '/time', screenshot: '13-time' }
  ];
  
  for (const test of financePages) {
    console.log('
Test:', test.name);
    try {
      const page = await browser.newPage();
