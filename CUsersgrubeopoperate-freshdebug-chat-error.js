...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: 'C:\Users\grube\op\operate-fresh\test-screenshots\chat-debug-login.png', fullPage: true });
    ...
    console.log('Step 5: Waiting for redirect to complete...');
    const currentUrl = page.url();
    console.log('Current URL: ' + currentUrl);
    
    await page.screenshot({ path: 'C:\Users\grube\op\operate-fresh\test-screenshots\chat-debug-after-login.png', fullPage: true });

    console.log('Step 6: Waiting 10 seconds for page to fully load...');
    await new Promise(resolve => setTimeout(resolve, 10000));

    console.log('Step 7: Taking final screenshot...');
    await page.screenshot({ path: 'C:\Users\grube\op\operate-fresh\test-screenshots\chat-debug-final.png', fullPage: true });

    console.log('Step 8: Getting page content...');
    const pageContent = await page.content();
    const hasErrorBoundary = pageContent.includes('ErrorBoundary') || pageContent.includes('error boundary');
    ...
    fs.writeFileSync(
      'C:\Users\grube\op\operate-fresh\CHAT_ERROR_DEBUG.json',
      JSON.stringify({
        url: currentUrl,
        hasErrorBoundary,
    ...
    console.log('Error-related messages: ' + errorBoundaryMessages.length);
    console.log('Final URL: ' + currentUrl);
    console.log('Has error boundary: ' + hasErrorBoundary);

  } catch (error) {
    console.error('Test failed:', error.message);
    await page.screenshot({ path: 'C:\Users\grube\op\operate-fresh\test-screenshots\chat-debug-error.png', fullPage: true });
