...
async function testChat(page) {
  console.log('
💬 TESTING: Chat/AI Features (CRITICAL)');
  console.log('─'.repeat(50));
  ...
  logTest('Chat', 'Chat input field exists', hasChatInput ? 'pass' : 'fail');
  ...
  if (hasChatInput) {
    try {
      ...
      const inputSelector = await page.evaluate(() => {
        ...
        const textInput = document.querySelector('input[type="text"][placeholder*="message"]');
        const chatInput = document.querySelector('[class*="chat-input"]');
        
        if (textarea) return 'textarea';
        if (textInput) return 'input[type="text"][placeholder*="message"]';
        if (chatInput) return '[class*="chat-input"]';
        return null;
      });

      if (inputSelector) {
        await page.click(inputSelector);
        await page.type(inputSelector, 'Hello, this is a comprehensive E2E test message. Can you confirm you received this?');
        await new Promise(r => setTimeout(r, 1000));
        await screenshot(page, '41-chat-message-typed');
        logTest('Chat', 'Type message in chat input', 'pass', 'Message typed successfully');
        
        // Check for send button
        const hasSendBtn = await elementExists(page, 'button[type="submit"], button:has-text("Send"), [class*="send-button"]', 3000);
        logTest('Chat', 'Send button exists', hasSendBtn ? 'pass' : 'fail');
        
        if (hasSendBtn) {
          // Try to click send button
          const sendClicked = await safeClick(page, 'button[type="submit"]', 2000) ||
                              await safeClick(page, 'button:has-text("Send")', 2000) ||
                              await safeClick(page, '[class*="send-button"]', 2000);
          
          if (sendClicked) {
            await new Promise(r => setTimeout(r, 3000));
            await screenshot(page, '42-chat-message-sent');
            logTest('Chat', 'Send message', 'pass', 'Message sent');
            
            // Wait for AI response
            await new Promise(r => setTimeout(r, 5000));
            await screenshot(page, '43-chat-ai-response');
            logTest('Chat', 'Wait for AI response', 'pass', 'Waited 5s for response');
          } else {
            logTest('Chat', 'Send message', 'fail', 'Could not click send button');
          }
        }
      } else {
        logTest('Chat', 'Find chat input selector', 'fail', 'Could not determine input selector');
      }
    } catch (error) {
      logTest('Chat', 'Send message flow', 'fail', error.message);
    }
  }
