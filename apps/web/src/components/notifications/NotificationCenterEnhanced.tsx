...
type FilterType = 'all' | 'invoice' | 'payment' | 'expense' | 'task' | 'document' | 'tax' | 'client' | 'ai' | 'system'
...
export function NotificationCenterEnhanced() {
  ...
  const filteredNotifications = notifications.filter((notification) => {
    ...
    return getCategoryFromType(notification.type) === filter
  })
  ...
  const categoryCounts = {
    ...
    invoice: notifications.filter((n) =>
      ['INVOICE_PAID', 'INVOICE_OVERDUE', 'INVOICE_DUE'].includes(n.type)
    ).length,
    payment: notifications.filter((n) => n.type === 'PAYMENT_RECEIVED').length,
    expense: notifications.filter((n) =>
      ['EXPENSE_APPROVED', 'EXPENSE_REJECTED'].includes(n.type)
    ).length,
    ...
    system: notifications.filter((n) =>
      ['SYSTEM_UPDATE', 'SYSTEM_ALERT', 'SYSTEM'].includes(n.type)
    ).length,
  }
          ...
          <Select value={filter} onValueChange={(value) => setFilter(value as FilterType)}>
            ...
            <SelectContent>
              <SelectItem value="all">
                All <Badge variant="secondary" className="ml-2">{categoryCounts.all}</Badge>
              </SelectItem>
              ...
              <SelectItem value="expense">
                Expenses <Badge variant="secondary" className="ml-2">{categoryCounts.expense}</Badge>
              </SelectItem>
              ...
              <SelectItem value="client">
                Clients <Badge variant="secondary" className="ml-2">{categoryCounts.client}</Badge>
              </SelectItem>
              <SelectItem value="ai">
                AI <Badge variant="secondary" className="ml-2">{categoryCounts.ai}</Badge>
              </SelectItem>
