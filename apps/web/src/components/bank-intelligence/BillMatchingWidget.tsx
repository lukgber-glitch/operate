...
interface BillMatchingWidgetProps {
  className?: string;
}

export function BillMatchingWidget({ className }: BillMatchingWidgetProps) {
  ...
  const handleConfirmMatch = (transactionId: string, billId: string) => {
    confirmMatchMutation.mutate({ transactionId, billId });
  };

  const outgoingPayments = unmatched?.outgoing || [];
  ...
  if (isError) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Bill Matching</CardTitle>
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>No unmatched outgoing payments</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900 text-center">
            ...
            <p className="text-xs text-green-700 dark:text-green-300">
              All outgoing payments are matched to bills
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card className={className}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription>
              {outgoingPayments.length} unmatched outgoing payment{outgoingPayments.length !== 1 ? 's' : ''}
            </CardDescription>
          </div>
          <Link
            href="/reconciliation"
      ...
      <CardContent>
        <div className="space-y-3">
          {outgoingPayments.slice(0, 3).map((payment) => (
            <div
              key={payment.id}
          ...
          {outgoingPayments.length > 3 && (
            <div className="text-center">
              ...
              >
                View all {outgoingPayments.length} unmatched payments
              </Link>
            </div>
          )}
        </div>
