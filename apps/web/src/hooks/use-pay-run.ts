...
export function usePayRun() {
  ...
  return {
    ...
    canProceed: (): boolean => {
      ...
      return store.isStepComplete(currentStep);
    },

    getTotalAdditions: (employeeUuid?: string): number => {
    ...
    getTotalDeductions: (employeeUuid?: string): number => {
    ...
    getEmployeeGrossPay: (employeeUuid: string): number => {
      ...
      // Add additions
      gross += store.getTotalAdditions(employeeUuid);

      return gross;
    },

    getEmployeeNetPay: (employeeUuid: string): number => {
      const gross = store.getEmployeeGrossPay(employeeUuid);
      ...
      const deductions = store.getTotalDeductions(employeeUuid);
    ...
    getTotalGrossPay: (): number => {
      return store.selectedEmployees.reduce(
        (sum, uuid) => sum + store.getEmployeeGrossPay(uuid),
        0
      );
    },
    ...
    getTotalNetPay: (): number => {
      return store.selectedEmployees.reduce(
        (sum, uuid) => sum + store.getEmployeeNetPay(uuid),
        0
      );
    },
