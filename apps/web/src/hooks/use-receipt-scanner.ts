...
export function useReceiptScan(scanId: string | null, enablePolling: boolean = true) {
  ...
  const fetchScan = useCallback(async () => {
    ...
    try {
      const response = await apiClient.get<ReceiptScan>(`/api/finance/receipts/scan/${scanId}`);
...
export function useConfirmScan() {
  ...
  const confirmScan = useCallback(async (scanId: string, data: ConfirmScanRequest): Promise<CreatedExpense> => {
    setIsConfirming(true);
    setError(null);

    try {
      const response = await apiClient.post<CreatedExpense>(`/api/finance/receipts/scan/${scanId}/confirm`, data);
