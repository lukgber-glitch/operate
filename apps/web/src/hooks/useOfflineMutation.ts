...
import { getDB, EntityType } from '@/lib/offline/db';
    ...
    async (item: TData, operation: 'create' | 'update' | 'delete') => {
      try {
        ...
        if (operation === 'delete') {
          ...
          await db.put(entityType, itemWithMeta);
        }
      } catch (err) {
 ...
 */
export function useOfflineCreate<TData extends { id: string }, TVariables>(
  entityType: EntityType,
...
export function useOfflineDelete<TVariables extends { id: string }>(
  ...
  options?: Partial<Omit<OfflineMutationOptions<void, TVariables>, 'optimisticUpdate'>>
) {
