...
export async function batchUpdateCache<T extends { id: string }>(
  ...
  try {
    ...
    await Promise.all([
      ...
      ...updates.map(entity => tx.store.put(entity as any)),
      tx.done,
    ]);
  } catch (err) {
...
export async function getEntityCountByStatus(
  ...
  try {
    const db = await getDB();
    // Note: 'by-status' index available on invoices, expenses, syncQueue
    // contacts uses 'by-type' instead - caller should use appropriate index
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const count = await db.countFromIndex(entityType, 'by-status' as any, status);
    return count;
  } catch (err) {
