...
const SCREENSHOTS_DIR = path.join(__dirname, 'test-screenshots/live-e2e');
...
async function runTest() {
  ...
  try {
    ...
    // Fill login form
    await page.type('#email', 'browsertest@test.com');
    ...
    await page.goto(`${BASE_URL}/finance/invoices`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    await page.goto(`${BASE_URL}/finance/expenses`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    await page.goto(`${BASE_URL}/time`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await new Promise(r => setTimeout(r, 2000));
  ...
  fs.writeFileSync(
    path.join(__dirname, 'LIVE_E2E_RESULTS.json'),
    JSON.stringify(report, null, 2)
  );

  console.log('\nReport saved to: LIVE_E2E_RESULTS.json');
}
